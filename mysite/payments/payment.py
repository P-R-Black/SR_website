...
class StripePayment:
    ...
    def __init__(self) -> None:
        ...
        self.publishable_key = settings.STRIPE_PUBLIC_KEY

    def get_or_create_item(self, userId: int, data: PaymentData):
        paymentItem = stripe.PaymentIntent.search(query=f"metadata['user_id']:'{userId}' AND status: 'requores_payment_method'")
...
def stripeWebhook(request):
    ...
    try:
        event = stripe.Webhook.construct_event(
            data, request.hearders.get('stripe-signature'), StripePayment().endpoint_secret
        )
