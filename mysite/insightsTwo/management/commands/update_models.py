...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        all_symbols = ['aero_def', 'airlines', 'auto_comps', 'autos', 'banking', 'beverages', 'biotechs',
                        'building_prods', 'cap_markets', 'chemicals', 'comm_equipment', 'comm_serv_supps',
                        'comm_services', 'con_stapes', 'cons_disc', 'cons_finance', 'const_materials',
                        'construction_engineering', 'container_packaging', 'div_telecom', 'divers_financial',
                        'diversified_utilities', 'elec_equip_instr_comps', 'electric_equipment', 'energy',
                        'energy_equip', 'entertainment', 'equity_real_estate', 'finance_sect', 'food_products',
                        'food_stapes', 'freight_logist', 'hc_prov_srvc', 'hc_supp_equip', 'healthcare_sect',
                        'hot_res_lies', 'household', 'household_prods', 'industrial_conglomerate',
                        'industrial_sect', 'info_tech_services', 'info_technology', 'insurance',
                        'intActive_med', 'intDirRetail', 'life_science', 'machinary', 'materials', 'media',
                        'metals_mining', 'multiline_retail', 'oil_gas_consum', 'pharma', 'professional_services',
                        'real_estate_sect', 'regulated_electric', 'regulated_gas', 'road_and_rail', 
                        'semis_and_equip', 'software', 'sp_500', 'spec_retail', 'text_app_lux', 
                        'trading_companies_distr', 'utilities']
        
        sector_dict = {
            'SP500 Communication Serv Mean': 3, 'Consumer Staples Mean': 4, 'Consumer Disc Mean': 2,
            ...
            'SP500 Regulated Electric Mean': 53, 'SP500 Diversified Utilities Mean': 52, 
            'SP500 Regulated Gas Mean': 54, 'SP500 Equity Real Estate Investment Mean': 51, 
            'SP500 Chemicals Mean': 47, 'SP500 Construction Materials Mean': 48,
            'SP500 ContainersAndPackaging Mean': 49, 'SP500 Metals and Mining Mean': 50,
            'SP500 Communication Equip Mean': 41, 'SP500 ElecEquip Instr Comps Mean':42,
            'SP500 IT Services Mean':43, 'SP500 Semis Mean': 44, 'SP500 Software Mean':45,
            'S&P 500 Aerospace & Defense Mean': 29, 'S&P 500 Freight & Logistics Mean': 30,
            'S&P 500 Airlines Mean': 31, 'S&P 500 Building Products Mean': 32,
            'S&P 500 Commercial Services & Supplies Mean': 33,
            'S&P 500 Construction and Engineering Mean': 34, 'S&P 500 Electric Equipment Mean': 35,
            'S&P 500 Industrial Conglomerates Mean': 36, 'S&P 500 Machinery Mean': 37,
            'S&P 500 Professional Services Mean': 38, 'S&P 500 Road and Rail Mean': 39,
            'S&P 500 Trading Companies and Distribution Mean': 40,
            'SP500 Biotechnology Mean': 24, 'SP500 HealthCare Equip Supps Mean': 25,
            'SP500 Health Provider and Svc Mean': 26,'SP500 LifeScienceTools Mean': 27,
            'SP500 Pharmaceuticals Mean': 28,'S&P 500 Banking Industry Mean': 20,
            'S&P 500 Capital Markets Mean':21, 'S&P 500 Consumer Finance Mean': 22,
            'S&P 500 Insurance Mean': 23, 'SP500 Energy Equip & Svc Mean': 18,
            'SP500 Oil Gas & Consumable Mean': 19,'SP500 Beverages Mean': 57, 'SP500 Food&Staples Mean': 61, 
            'SP500 Food Products Mean': 60, 'SP500 Household Products Mean':64, 'SP500 Auto Components Mean': 55, 
            'SP500 Automobile Mean': 56,'SP500 Hotel Rest Leis Mean': 62, 'SP500 Household Durables Mean': 63,
            'SP500 Internet & Direct Marketing Retail':14, 'SP500 Multiline Retail Mean': 15, 
            'SP500 Specialty Retail Mean': 16, 'SP500 Textiles, Apparel & Luxury Goods Mean': 17,
            'SP500 Diversified Telecom Mean':58, 'SP500 Entertainment Mean': 59, 
            'SP500 Interactive Media Mean': 65, 'SP500 Media Mean': 66,
        }
        
        sector_to_industry_lookup = {
            14: 2 , 15: 2, 16: 2, 17: 2, 18: 1, 19: 1, 20: 5 , 21: 5, 22: 5 , 23: 5, 24: 6, 
            25: 6, 26: 6, 27: 6, 28: 6, 29: 7 , 30: 7, 31: 7, 32: 7, 33: 7, 34: 7, 35: 7, 
            36: 7, 37: 7, 38: 7, 39: 7, 40: 7, 41: 7, 42: 8, 43: 8, 44: 8, 45: 8, 47: 9, 
            48: 9, 49: 9, 50: 9, 51: 10, 52: 11, 53: 11, 54: 11, 55: 2, 56: 2, 57: 4, 58: 3, 
            59: 3, 60: 4, 61: 4, 62: 2, 63: 2, 64: 4, 65: 3, 66: 3
            }
        ...
        industry_names = [
            ...
            'Specialty Retail', 'Textiles, Apparel, and Luxury Goods', 'Trading Companies & Distributors'
            ]
        ...
        industry_dict = {
            ...
            'Specialty Retail': 16, 'Textiles, Apparel, and Luxury Goods': 17, 
        ...
        ind_name = IndustryName.objects.all()
        ...
        db_ind_name = set()
        new_list_of_ind_dates = list()
        for gws in get_weekly_stats:
            if "2023" in gws['fields']['date_created'][:10]: 
                if gws['fields']['industry_name'] in sector_to_industry_lookup.keys():
                    db_ind_name = (str(gws['fields']['industry_name']), str(gw['fields']['date_created'][:10]))
                    new_list_of_ind_dates.append(db_ind_name)
        # print('new_list_of_ind_dates', new_list_of_ind_dates)
        ...
        for i in get_weekly_stats:
            ...
            if i['fields']['industry_name'] in sector_to_industry_lookup.keys():
                all_industry_numbers = i['fields']['industry_name']
                if all_industry_numbers not in pks_new_list :
                    pks_new_list.append(all_industry_numbers)
                
        pks_new_list = [x for x in pks_new_list if x is not None]
        # print('pks_new_list', pks_new_list)
        ...
        while len(pks_new_list) > 0:
            ...
            this_weeks_csv = "Stats_2023-07-29.csv"
            with open(this_weeks_csv, "r") as csvfile:
                datareader = csv.reader(csvfile)
                next(datareader, None)
                new_stats_list = []
                for d in datareader:
                    new_stats_list.append(d)
                # print('new_stats_list', new_stats_list)
            ...
            if check_for_previous_entry in new_list_of_dates:
                ...
                if pk_num <= 13:
                    ...
                    sect_pk = sector_to_industry_lookup[pk_num]
                    print('sect_pk', sect_pk)
                    print('pk_num', pk_num)
                    models = WeeklyStats(industry_name=IndustryName.objects.get(pk=pk_num), sector_name=SectorName.objects.get(pk=sect_pk), price=target_stat[1], forward_pe=target_stat[2], 
                                        trailing_pe=target_stat[3], forward_ps=target_stat[4], trailing_ps=target_stat[5], 
                                        mrq_pb=target_stat[6], pcf=target_stat[7], quick_ratio=target_stat[8], 
                                        current_ratio=target_stat[9], total_debt_to_equity=target_stat[10], 
                                        total_debt_to_capital=target_stat[11], roe=target_stat[12], roa=target_stat[12], 
        ...
        # df = pd.read_csv('cap_markets.csv')
        # df.replace("nan", 0.00)
        # trav_file = df.T
